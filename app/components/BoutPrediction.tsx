...
export function BoutPrediction({
  ...
  return (
    <section className="bout-prediction" aria-label={`${leftKo} 대 ${rightKo} 승부예측`}>
      <header><span>승부예측</span><small>{totalVotes}명 참여</small></header>
      <div>
        <button type="button" data-selected={selectedPick === left} onClick={() => choose(left)}>
          <strong>{leftKo}</strong><b>{percentage(leftVotes)}%</b>
        </button>
        <button type="button" data-selected={selectedPick === right} onClick={() => choose(right)}>
          <strong>{rightKo}</strong><b>{percentage(rightVotes)}%</b>
        </button>
      </div>
      <p>이 브라우저에서는 선택을 다시 바꿀 수 있습니다.</p>
